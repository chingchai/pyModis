...
def modisdownload(conf,inputs,outputs):
    ...
    outputs["Result"]["value"]=\
            "dns: "+inputs["dns"]["value"]+ " path: "+inputs["path"]["value"]+ " tiles: "+inputs["tiles"]["value"]+ " today: "+inputs["today"]["value"]+ " enddate: "+inputs["enddate"]["value"]+ " product: "+inputs["product"]["value"]

    # outputs["Result"]["value"]=\
    #         "Hello "+inputs["name"]["value"]+" from the ZOO-Project Python "+str(sys.version)+" world !"
    # +"from the ZOO-Project Python "+str(sys.version)+ "and pyModis!"+str(glob.glob(dns + '*.hdf'))"
